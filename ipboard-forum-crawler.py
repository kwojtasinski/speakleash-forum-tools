...
from usp.tree import \
    sitemap_tree_for_homepage  # install ultimate-sitemap-parser
...
DATASET_URL: str = args.DATASET_URL or "https://forum.omegaklub.eu"
...
EXPECTED_URL_PARTS = ['/topic','/temat', '/thread', '/forum', '/threads'] # we're targeting the full topics to optimize the performance
...
def get_item_text(url: str) -> str:
    ...
    if response and response.ok:
        ...
        try:
            ...
            comment_blocks = soup.find_all("div", class_="post entry-content")
        except Exception as e:
        ...
        #Process next pages
        if soup.find_all('li', class_='next'):
            try:            
                # Sleep for convinience
                time.sleep(TIME_SLEEP)
                
                # Iterate through all of the pages in given topic/thread
                while len(soup.find_all("div", class_="post entry-content"))>0 and \
                    len(soup.find_all('li', class_='next'))==0:
                    
                    next_page_btns = soup.find_all('li', class_='next')
...
def main():
    ...
    try:
        ...
        sys.exit()
    logging.info("*** Scraping is done -> preparing merge archives ***")
    ...
    logging.info("*** Archives merge is done -> preparing dataset manifest... ***")
    ...
    if create_manifest(file_name_manifest, total_docs_archive, file_size) == False:
        logging.error("*** Error while creating manifest ***")
...
if __name__ == '__main__':
    ...
    if main() == True:
        ...
        logging.warning("--- UPS SOMETHING WENT WRONG +++")
